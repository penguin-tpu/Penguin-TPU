...
def test_derive_roofline_metrics_matches_current_config() -> None:
    ...
    assert metrics.vmem_bandwidth_bytes_per_cycle == 64.0
    ...
    assert metrics.mxu_vmem_knee_ops_per_byte == 256.0
    assert metrics.vpu_dram_knee_ops_per_byte == 512.0
    assert metrics.vpu_vmem_knee_ops_per_byte == 16.0
...
def test_representative_kernel_points_match_current_machine() -> None:
    ...
    assert math.isclose(mxu.vmem_ceiling_ops_per_cycle, 2048.0)
    ...
    assert vpu.vmem_ceiling_ops_per_cycle == 16.0
...
def test_format_report_includes_normalized_and_scaled_metrics() -> None:
    ...
    assert "VMEM bandwidth: 64.00 B/cycle" in report
    ...
    assert "VMEM bandwidth: 32.000000 GB/s" in report
...
def test_pi0_workload_roofline_points_match_current_machine() -> None:
    ...
    assert math.isclose(top.vmem_ceiling_ops_per_cycle, 3955.4272189349113)
    ...
    assert math.isclose(aggregate.vmem_ceiling_ops_per_cycle, 3863.6353061642435)
