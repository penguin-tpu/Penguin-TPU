"""Run the fixed-shape Gemma attention example and emit a Perfetto trace."""
...
from examples.gemma_workloads import run_gemma_attention_example
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--trace",
        type=Path,
        default=Path("outputs/examples/gemma_attention_trace.json"),
        help="Path to the Perfetto-compatible JSON trace output.",
    )
    parser.add_argument(
        "--bundle-root",
        type=Path,
        default=Path("outputs/examples/bundles/gemma_attention"),
...
def main() -> int:
    ...
    result = run_gemma_attention_example(trace_path=args.trace, bundle_root=args.bundle_root)

    print("Gemma attention example completed.")
